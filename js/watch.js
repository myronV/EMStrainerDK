(function (g, h, E) {
	...
	function xa(a, b) {
		...
		if (.01 > Math.random()) try {
			var d = a && a.message || "";
			(c = -1 < d.indexOf("network error occurred") || -1 < d.indexOf("send beacon") && -1 < d.indexOf("Content Security Policy")) || (new ra).log("jserrs", Z, a.message, b, G.href, "", "string" === typeof a.stack && a.stack.replace(/\n/g, "\\n"))
		} catch (e) {
		}
	}
	...
	function pb(a, b, c, d, e) {
		...
		function w(a, b, c, d, e, f) {
			for (var M; c && (M = l.getElementSize(c)) && (!M[0] || !M[1]);) c = l.getElementParent(c);
			if (!c) return null;
			M = c[K];
			if (!M || 0 > M) return null;
			var m = {mousemove: 2, click: 32, dblclick: 33, mousedown: 4, mouseup: 30, touch: 12}[b];
			if (!m) return null;
		...
		function Q(a) {
			var b = a[K];
			if (!b || 0 > b || !/^INPUT|SELECT|TEXTAREA$/.test(a.nodeName) || !a.form || aa(a.form)) return null;
			...
			for (var e = -1, f = a.form.elements,
				     M = f.length, m = 0, g = 0; m < M; m++) if (f[m].name == a.name) {
				if (f[m] == a) {
					e = g;
					break
				}
		...
		function T(a, b) {
			...
			if (0 > c) return null;
			for (var d = b.elements, e = d.length, M = [], m = 0; m < e; m++) if (!l.isEmptyField(d[m])) {
				var g = d[m][K];
		...
		function x(a) {
			if (a[K]) a:{
				...
				if (0 < c) {
					...
					var e = Da[c], f = a[0] + "x" + a[1], m = a[2] + "x" + a[3];
				...
				Da[Ra] = {};
				Ra++;
				if (a.nodeName) if (c = +a[K], !isFinite(c) || 0 >= c) b = null; else {
					d = 64;
					e = 0;
					var g = l.getElementParent(a);
					f = g && g[K] ? g[K] : 0;
					0 > f && (f = 0);
					m = a.nodeName.toUpperCase();
					var M =
						vb[m];
					M || (d |= 2);
					...
					(g = g ? l.getElementRegion(g) : null) && u[0] == g[0] && u[1] == g[1] && u[2] == g[2] && u[3] == g[3] && (d |= 8);
					Da[c].pos = u[0] + "x" + u[1];
					Da[c].size = u[2] + "x" + u[3];
					...
					b:{
						...
						for (var k = 0; k < h.length; k++) if ((!h[k].id || "string" != typeof h[k].id) && l.calcAttribChecksum(h[k]) ==
							A && l.calcTextChecksum(h[k]) == g) {
							h = !0;
							break b
						}
						h = !1
		...
		function aa(a) {
			return a &&
				l.classNameExists(a, "(ym-disable-submit|-metrika-noform)")
		}

		function W(a) {
			var b = F.getTarget(a);
			if (b && "SCROLLBAR" != b.nodeName) {
				if (/^INPUT|SELECT|TEXTAREA|BUTTON$/.test(b.tagName)) if (b[K]) x(b); else {
					var c = b.form;
					if (c) {
						c = c.elements;
						for (var d = c.length, e = 0; e < d; e++) /^INPUT|SELECT|TEXTAREA|BUTTON$/.test(c[e].tagName) && !c[e][K] && x(c[e])
					} else x(b)
				} else x(b);
				C(w, [r(), a.type, b, F.getPos(a), a.which, a.button])
			}
		}
		...
		function la(a) {
			var b = u(), c = b - ab;
			if (!(10 > c)) {
				var d = F.getPos(a), e = Ta[0] - d[0], f = Ta[1] - d[1];
				e = e * e + f * f;
		...
		function N() {
			var a = l.getDocumentScroll(), b = u();
			10 > b - ra || 10 > Math.abs(a[0] - Z[0]) && 10 > Math.abs(a[1] -
				Z[1]) || (ra = b, Z = a, C(E, [r(), a]))
		}
		...
		function S(a) {
			if (a = F.getTarget(a)) {
				...
				if (a.localId) {
					if (b = da[a.localId], !b || 10 > Math.abs(c[0] - b[0]) && 10 > Math.abs(c[1] - b[1])) return
				} else {
					for (; da[b];) b = Math.random();
					a.localId = b
				}
				da[a.localId] = c;
				a !== h && (x(a), C(G, [r(), c, a]))
		...
		function ea(a) {
			return "INPUT" ==
			a.tagName ? "password" == a.type || a.name && V.test(a.name) || a.id && V.test(a.id) : !1
		}

		function ka(a) {
			if (!a) return {ignore: !0, forceRecord: !1, isPrivate: !1, privateFlag: !1, recordData: !0};
			var b = "password" === a.type || V.test(a.name) || V.test(a.id), c = f.reduce(function (b, c) {
		...
		function U(a, b, c) {
			a = F.getTarget(a);
			var d = ka(a);
			d.ignore || !d.forceRecord && (d.isPrivate && e.get("isEU") || fa(a)) || (x(a), C(ma, [r(), b, c, a, d.privateFlag]))
		}

		function y(a) {
			var b = a.keyCode, c = ba(a);
			if ({
				3: 1,
				8: 1,
				9: 1,
				...
				144: 1,
				145: 1
			}[b] || 112 <= b && 123 >= b || 96 <= b && 105 >= b || c & 16) 19 == b && 4 == (c & -17) && (b = 144), U(a, b, c | 16), na = !1, f.setTimeout(function () {
				na = !0
			}, 1, "wvl.keyDown"), !(67 == b && c & 4) || c & 1 || c & 2 || L()
		}

		function A(a) {
			na &&
			!oa && 0 !== a.which && (U(a, a.charCode || a.keyCode, ba(a)), oa = !0, f.setTimeout(function () {
				oa = !1
			}, 1, "vwl.keyPress"))
		}
			...
			if ((a = F.getTarget(a)) && /^INPUT|SELECT|TEXTAREA|BUTTON$/.test(a.tagName)) {
				if (a[K]) x(a); else {
					...
					if (b) {
						b = b.elements;
						for (var c = b.length,
							     d = 0; d < c; d++) /^INPUT|SELECT|TEXTAREA|BUTTON$/.test(b[d].tagName) && !b[d][K] && x(b[d])
					} else x(a)
				}
				C(P, [r(), a])
			}
		...
		function Ga(a) {
			(a = F.getTarget(a)) && /^INPUT|SELECT|TEXTAREA|BUTTON$/.test(a.tagName) && (x(a), C(sa, [r(), a]))
		}

		function Ha(a) {
			if ((a = F.getTarget(a)) && !ea(a) && !fa(a) && /^INPUT|SELECT|TEXTAREA$/.test(a.tagName)) {
				var b = ka(a), c = /^(checkbox|radio)$/.test(a.type) ? a.checked : a.value;
				c = b.recordData ? c : f.map(function () {
		...
		function za(a) {
			if ((a =
				F.getTarget(a)) && !aa(a) && "FORM" == a.nodeName) {
				for (var b = a.elements, c = 0; c < b.length; c++) l.isEmptyField(b[c]) || x(b[c]);
				C(T, [r(), a], !0)
			}
		}
		...
		function Ua(a) {
			...
			if (a.touches && a.touches.length) {
				...
				if (b && b != h) {
					x(b);
					for (var c = 0; c < a.touches.length; c++) C(w, [r(), "touch", b, [a.touches[c].pageX, a.touches[c].pageY], 0, 0])
				}
			}
		}
		...
		var va = new Ja({
			...
			counterId: b,
			counterType: c,
			meta: {url: D().href, hitId: d, timezone: Aa, timestamp: Ba}
		}), eb = l.getBody(), vb = {
			A: 1,
			ABBR: 2,
			ACRONYM: 3,
		...
		m(0);
		var Ra = 1, ab = 0, Ta = [0, 0], ra = 0, Z = [0, 0], da = {}, V = /^(password|passwd|pswd)$/, na = !0, oa = !1,
			qa = "", ia = !1, ya = !0, ja = u(), K = "metrikaId_" + Math.random(), Da = {},
		...
		if (!f.isMetrikaPlayer()) {
			...
			k.on(h, "mouseup", ca);
			k.on(g, "scroll", N);
			if ("onmousewheel" in h) k.on(h, "mousewheel", Ia); else k.on(h, "wheel", Ia);
			k.on(g, "beforeunload",
				ta);
			if (!wb) k.on(g, "unload", ta);
			k.on(g, "resize", ha);
			k.on(h, "keydown", y);
			k.on(h, "keypress", A);
			k.on(h, "copy", L);
			k.on(h, "touchmove,touchstart", Ua);
			if (eb) k.on(h, "mouseleave", db);
			h.attachEvent && !g.opera ? (k.on(h, "focusin", X), k.on(h, "focusout", cb)) : (k.on(g, "focus", H), k.on(g, "blur", O), k.on(h, "blur", O));
			h.addEventListener ? (k.on(h, "scroll", S), k.on(h, "focus", Fa), k.on(h, "blur", Ga), k.on(h, "change", Ha), k.on(h, "submit", za)) : h.attachEvent && (k.on(h, "focusin", Fa), k.on(h, "focusout", Ga), function () {
				for (var a = h.getElementsByTagName("form"),
					     b = 0; b < a.length; b++) {
					for (var c = a[b].getElementsByTagName("*"), d = 0; d < c.length; d++) if (/^INPUT|SELECT|TEXTAREA$/.test(c[d].tagName)) k.on(c[d], "change", Ha);
					k.on(a[b], "submit", za)
				}
			}());
			(function () {
				...
				if (a.length) for (var b = 0; b < a.length; b++) {
					var c = a[b].submit;
					if ("function" === typeof c || "object" === typeof c && /^\s*function submit\(\)/.test(String(c))) a[b][Ka] = c, a[b].submit = function () {
						za({target: this});
						return this[Ka]()
					}
				}
			})();
			"0:0" !== l.getDocumentScroll().join(":") && N();
			ha()
		}
		return {
			start: function () {
			...
			stop: function () {
				...
				h.removeEventListener ?
					(k.un(h, "scroll", S), k.un(h, "focus", Fa), k.un(h, "blur", Ga), k.un(h, "change", Ha), k.un(h, "submit", za)) : h.detachEvent && (k.un(h, "focusin", Fa), k.un(h, "focusout", Ga), function () {
					for (var a = h.getElementsByTagName("form"), b = 0; b < a.length; b++) {
						for (var c = a[b].getElementsByTagName("*"), d = 0; d < c.length; d++) /^INPUT|SELECT|TEXTAREA$/.test(c[d].tagName) && k.un(c[d], "change", Ha);
						k.un(a[b], "submit", za)
					}
				}());
				(function () {
					for (var a = h.getElementsByTagName("form"), b = 0; b < a.length; b++) a[b][Ka] && (a[b].submit = a[b][Ka])
				})()
			}, uploadPages: function (a,
			                          b) {
				function c() {
					k.un(h, "DOMContentLoaded", c);
					k.un(g, "load", c);
					for (var e = a.split(/\n/), f = D().href, m = /regexp:/, u = 0; u < e.length; u++) {
						var y = e[u];
						if (y) if (m.test(y)) {
							y = La(y.replace(m, ""));
							try {
				...
				var m = g.XMLHttpRequest ? new g.XMLHttpRequest : new ActiveXObject("Msxml2.XMLHTTP"),
					u = l.getDocumentCharset(), y = "text/html" + (u ? ";charset\x3d" + u : ""),
					A = new xb({protocol: a, counterId: b, counterType: c});
				if ("html" === e) return e = new RegExp("\x3cscript [^\x3e]*?//" + I.host.replace(/\./g, "\\\\.") + "/watch/.*?\x3c/script\x3e", "gi"), A.sendContent(l.getDocumentHTML().replace(e, ""), y, d, Aa, Ba), !0;
				m && (m.open("get", D().href, !0), m.onreadystatechange = v(function () {
					if (4 ===
						m.readyState) {
				...
				}, "updatePage.1"), m.overrideMimeType && u && m.overrideMimeType(y), m.send(null));
				return !0
			}, "uploadPage")
		}
	}
	...
	function Ma() {
		return -1 != D().hostname.search(/(?:^|\.)(?:ya|yandex)\.(?:\w+|com?\.\w+)$/)
	}
	...
	function gb(a, b) {
		...
		if (!a || !b) return !1;
		var d = [];
		for (c = 0; c < b.length; c++) d.push(b[c].replace(/\^/g, "\\^").replace(/\$/g, "\\$").replace(/\./g, "\\.").replace(/\[/g, "\\[").replace(/\]/g, "\\]").replace(/\|/g, "\\|").replace(/\(/g, "\\(").replace(/\)/g,
	...
	function zb(a, b) {
		return (a ?
			a.replace(/^www\./, "") : "").toLowerCase() == (b ? b.replace(/^www\./, "") : "").toLowerCase()
	}
	...
	function hb() {
		var a = g.performance || g.webkitPerformance, b = a && a.timing, c, d = [];
		if (b && (c = b.navigationStart)) for (d = [b.domainLookupEnd - b.domainLookupStart, b.connectEnd - b.connectStart, b.responseStart - b.requestStart, b.responseEnd - b.responseStart, b.fetchStart - c, b.redirectEnd -
		b.redirectStart, b.redirectCount || a.navigation && a.navigation.redirectCount], d.push(b.domInteractive && b.domLoading ? b.domInteractive - b.domLoading : null), d.push(b.domContentLoadedEventStart && b.domContentLoadedEventEnd ? b.domContentLoadedEventEnd - b.domContentLoadedEventStart : null), d.push(b.domComplete ? b.domComplete - c : null), d.push(b.loadEventStart ? b.loadEventStart - c : null), d.push(b.loadEventStart && b.loadEventEnd ? b.loadEventEnd - b.loadEventStart : null), d.push(b.domContentLoadedEventStart ? b.domContentLoadedEventStart -
			c : null), a = 0; a < d.length; a++) b = d[a], null !== b && (0 > b || 36E5 < b) && (d[a] = null);
		return d
	}

	function Bb(a) {
		var b = [], c = a._lastPerformanceTiming, d = hb(), e;
		if (c) {
			var f = 0;
	...
	function Db() {
		var a = g.performance && g.performance.getEntriesByType ? f.filter(Cb, g.performance.getEntriesByType("paint")) : [];
		if (a.length) return a = a[0], Math.round(a.startTime);
		if ("object" == typeof g.chrome && g.chrome.loadTimes) {
			if (a = g.chrome.loadTimes(), a.requestTime && a.firstPaintTime && g.performance && g.performance.timing) return Math.round(1E3 * a.firstPaintTime - g.performance.timing.navigationStart)
	...
	var k = B.inherit({
		_initComponent: function () {
			...
			e = k._getOpts(e);
			b = b.split(",");
			var f = b.length, u = this._listeners, h;
			for (h = 0; h < f; h++) {
				var l = b[h];
				var p = v(function (a) {
					c.call(d || this, a || g.event)
				}, "on" + l + e.name);
				u[u.length] = [a, l, c, d, e, p];
				a.addEventListener ? a.addEventListener(l, p, k._prepOpts(e)) : a.attachEvent && a.attachEvent("on" + l, p)
			}
		}, un: function (a, b, c, d, e) {
			e = k._getOpts(e);
			var m = this._listeners, g = m.length, h;
			for (h = 0; h < g; h++) {
				var l = m[h];
	...
	k.supportsPassive = function () {
		if (k._supportsPassive !== E) return k._supportsPassive;
	...
	var w = {
		...
		getMs: function () {
			return 0 < this.UNLOAD_MS ? this.UNLOAD_MS : this.isPerfomance ?
				Math.round(g.performance.timing.navigationStart + g.performance.now()) : +new Date
		}
	};
	...
	var Ca = B.inherit({
		...
		_initComponent: function () {
			...
			this._flushTID = null;
			this._saveToStorageThrottled = f.throttle(this._saveToStorage,
				300, this, "DataBuffer._saveToStorage");
			this.storage && ((a = this.storage.get(this.storageKey)) && a.data && a.meta && a.time && a.time + this.bufferExpireTime > w.getMs() && this.onFlush.call(this.onFlushCtx || this, a.data, a.meta, a.pnum), this.clear())
		},
		...
		append: function (a, b) {
			...
			this.active && ((b || this.getFlushSize() >= this.maxBufferSize) && this._flush(), this._flushTID || (this._flushTID = f.defer(this._flush, this.flushTimeout,
				this, [], "DataBuffer._flush")))
		},
		activate: function () {
		...
		_saveToStorage: function () {
			this.storage && this._data.length && this.storage.set(this.storageKey, {
				data: this._data, meta: this.meta, pnum: this._packetNumber,
				time: w.getMs()
			})
		}
	}), t = {
	...
	f.async = function (a, b, c) {
		if (g.postMessage) {
			var d = "__ym__promise" + Math.random() +
				"_" + w.getMs();
			var e = function (c) {
				c.data === d && (c.stopPropagation && c.stopPropagation(), a.call(b), k.un(g, "message", e, null, {passive: !1}))
			};
			k.on(g, "message", e, null, {passive: !1, name: "async" + c});
			g.postMessage(d, "*")
	...
	var ja = B.inherit({
		transports: [], globalConfig: t, postParams: [], isDisabled: function () {
			return this.globalConfig.get("oo")
		}, send: function (a, b, c, d) {
			var e = [a], g = this, h = c || function () {
			};
			g.isDisabled() ? h.call(d) : function p(a) {
				if (a < this.transports.length) try {
					...
					(function rb(m) {
						m >= e.length ? c.blockNext() || p.call(this, a + 1) : c.request(e[m], b, function (a, b, c) {
							a ? h.call(d, b, c) : f.async(function () {
								rb.call(g, m + 1)
							})
						}, this)
					}).call(this, 0)
				} catch (qb) {
					xa(qb, "send by " + (c && c.id)), p.call(this, a + 1)
				}
			}.call(this, 0)
		}
	}), ca = {};
	...
	f.map = function (a, b) {
		var c, d = [];
		for (c =
			     0; c < b.length; c++) d.push(a(b[c], c));
		return d
	};
	ca.stringify = function (a, b) {
		var c = [], d = b || {}, e = {}, g = f.keys(a).length, h, r;
		if (d.lastOrdering !== E) for (r = 1; r <= b.lastOrdering.length; r++) e[b.lastOrdering[r - 1]] = g + r;
		if (d.firstOrdering !== E) for (r = 0; r < b.firstOrdering.length; r++) e[b.firstOrdering[r]] = r;
		for (h in a) if (a.hasOwnProperty(h)) if (d = a[h], f.isArray(d)) for (r = 0; r < d.length; r++) c.push({
			key: h,
			value: encodeURIComponent(h) + "\x3d" + encodeURIComponent(String(d[r]))
		}); else c.push({
			key: h, value: encodeURIComponent(h) + "\x3d" +
	...
	var S = B.inherit({
		...
		id: "XHR", index: 7, method: "POST", withCredentials: !0, request: function (a, b, c, d) {
			...
			if (/[^a-z0-9.:-]/.test(G.host) && g.opera && "function" === typeof g.opera.version) {
				var h = g.opera.version();
				if ("string" === typeof h && "12" === h.split(".")[0]) return c.call(d,
					!1)
			}
			if (g.XMLHttpRequest) {
	...
	var Eb = S.inherit({
		id: "beacon", index: 1, request: function (a, b, c, d) {
			...
			!n.isAndroidWebView() && navigator.onLine ? (b = this._splitParams(b), b.get["force-urlencoded"] = 1, c.call(d, navigator.sendBeacon(this._buildUrl(a, b.get), ca.stringify(b.post)))) : c.call(d, !1)
		}
	});
	...
	var ea = S.inherit({
		...
		request: function (a, b, c, d) {
			...
			h.style.top = "-99999px";
			var r = ['\x3ciframe name\x3d"', g, '"\x3e\x3c/iframe\x3e', '\x3cform action\x3d"', this._buildUrl(a, b.get), '" method\x3d"post" target\x3d"', g, '" enctype\x3d"', this.enctype,
				'"\x3e'];
			f.forEachKey(b.post, function (a) {
				f.mergeArrays(r, ['\x3cinput type\x3d"hidden" autocomplete\x3d"off" autocorrect\x3d"off"', ' autocapitalize\x3d"off" spellcheck\x3d"false" name\x3d"', a, '"/\x3e'])
			});
	...
	}), Fb = ea.inherit({id: "htmlfile", index: 3, htmlfileOnly: !0}), V = S.inherit({
	...
	f.toJSON = function (a) {
		...
		switch (a.constructor) {
			...
			case Object:
				c = [];
				for (d in a) a.hasOwnProperty(d) && a[d] !== E && (c[c.length] = f.toJSON(d) + ":" + f.toJSON(a[d]));
				return "{" + c.join(",") + "}";
			default:
	...
	f.getTld = function () {
		if (!this._tld) {
			var a = G.hostname.split(".");
			this._tld = a[a.length - 1]
		}
		return this._tld
	};
	var Q = {abc: "ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789", tail: "+/\x3d", tailSafe: "*-_"},
		l = {
	...
	J.get = function (a) {
		return (new J({onlyCurrentDomain: !0})).read(a)
	};
	J.set = function (a, b, c) {
		return (new J({onlyCurrentDomain: !0})).create(a, b, c)
	};
	J.isEnabled = function () {
		return (new J({onlyCurrentDomain: !0})).isEnabled()
	};
	var P = ja.inherit({
		protocol: "",
		host: I.host,
		...
		_sendSavedRequest: function (a, b, c, d, e) {
			var g = this.globalConfig.get("firstReqStatus"), h = b.forceNoRedirect;
			if ("process" == g) this.globalConfig.set("firstReqCallbacks", f.mergeArrays(this.globalConfig.get("firstReqCallbacks") || [], [[this, arguments]])); else {
				...
				c.zz = Va.getZZ();
				c.zz || delete c.zz;
				g = [this.protocol, "//", this.host, "/", this.resource, "/", this.counterId, h ? "/1" : ""].join("");
				delete b.forceNoRedirect;
				var r = [];
				c && (f.forEachKey(c, function (a, b) {
					"t" != a && f.mergeArrays(r, [a,
						b])
	...
	P.initCookie = function () {
		var a = new J, b = a.read("uid");
		b || (b = w.getSeconds(), b = b + "" + f.random(), a.create("uid", b, 1036800));
		P._userID = b
	};
	P.retransmit = function (a) {
		var b = P._storage, c = b.get("retryReqs") || {}, d;
		b.remove("retryReqs");
		f.forEachKey(c, function (b, c) {
			var e = w.getMs();
			a.get("firstReqStatus") || a.set("firstReqStatus", "complete");
			c.ghid && c.ghid != a.get("_globalMetrikaHitId") && c.time && c.time +
			864E5 > e && 500 < e - c.time && 2 >= c.browserInfo.rqnl && (d = new P({
				protocol: c.protocol,
				host: c.host,
				resource: c.resource,
	...
	Q.encode = function (a, b) {
		var c = (Q.abc + (b ? Q.tailSafe : Q.tail)).split(""), d = a.length, e = [], g = d - d % 3, h;
		for (h = 0; h < g; h += 3) {
			var k = (a[h] << 16) + (a[h + 1] << 8) + a[h + 2];
			f.mergeArrays(e, [c[k >> 18 & 63], c[k >> 12 & 63], c[k >> 6 & 63], c[k & 63]])
		}
		switch (d - g) {
			case 1:
				k = a[g] << 4;
	...
	f.is4Bytes = function (a) {
		return !f.arrayEvery((encodeURIComponent(a).match(/(%[A-F,\d]{2})+/g) || []).join("").split("%"), function (a) {
	...
	n.getSilverlightVersion = function () {
		if (!n._silverlightVersion) if (g.ActiveXObject) try {
		...
		} else if (b = g.navigator, b = b.plugins && b.plugins["Silverlight Plug-In"]) n._silverlightVersion =
			b.description;
	...
	n.getFlashVersion = function () {
		if (!n._flashVersion) {
			...
			if ("undefined" != typeof a.plugins && "object" == typeof a.plugins["Shockwave Flash"]) {
				...
				var c = b.version;
				if ((b = b.description) || c) if (a = a.mimeTypes, "undefined" == typeof a || !a["application/x-shockwave-flash"] || a["application/x-shockwave-flash"].enabledPlugin) c ? n._flashVersion = c : b && (n._flashVersion = b.replace(/([a-zA-Z]|\s)+/, "").replace(/(\s+r|\s+b[0-9]+)/,
					"."))
			} else if ("undefined" != typeof g.ActiveXObject) try {
				if (c = new ActiveXObject("ShockwaveFlash.ShockwaveFlash"), b = c.GetVariable("$version")) n._flashVersion = b.split(" ")[1].replace(/,/g, ".").replace(/[^.\d]/g, "")
			} catch (d) {
			}
		}
	...
	n.getFingerPrint = function () {
		...
		if (navigator.mimeTypes && navigator.mimeTypes.length) for (b = 0; b < navigator.mimeTypes.length; b++) c = navigator.mimeTypes[b], f.mergeArrays(a, [c.type, c.description,
			c.suffixes]);
	...
	n.isDesktop = function (a) {
		var b = a.get("bt", {});
		a.get("bt") ===
		E && (b.p = navigator.getBattery ? navigator.getBattery() : null, a.set("bt", b), b.p && b.p.then && b.p.then(function (a) {
	...
	k.ready = function (a) {
		var b = h.readyState;
		if ("interactive" === b || "complete" === b) f.setTimeout(a, 0, "evt::ready"); else {
			var c = function () {
				k.un(h, "DOMContentLoaded", c);
				k.un(g, "load", c);
				a()
	...
	var ib = {
		url: I.protocol + "//" + I.host + "/metrika/", _value: null, init: function (a) {
			...
			k.ready(function () {
				...
				n.isSafari() && (b.style.position = "absolute", b.style.visibility = "hidden",
					b.style.width = "0px", b.style.height = "0px", l.getRootElement().appendChild(b))
			})
		}, _saveReq: function (a) {
			this._value = a = a ? "1" : "2";
	...
	Q.decode = function (a, b) {
		for (var c = Q.abc + (b ? Q.tailSafe : Q.tail), d = 0, e = "", f,
			     g, h, k, l; a.length % 4;) a += "\x3d";
		do {
	...
	var ka = P.inherit({
		...
		paramsQueue: {
			...
			_filter: function (a) {
				...
				var b = {};
				this.paramsQueue.length && (b = f.reduce(this._reduce(f.keys(a)), {}, this.paramsQueue), this.paramsQueue = f.filter(this._filter, this.paramsQueue));
				return f.mixin({}, a, b)
			}
		},
		...
		_initComponent: function () {
			...
			"0" === "" + this.counterType && (this.sendTitle =
				-1 === f.arrayIndexOf(this.countersNoTitle, "" + this.counterId))
		},
		...
		sendPrerenderHit: function (a) {
			this._hitExt({
				url: a.url, title: a.title, referrer: a.referrer,
				modes: {ar: !0, pq: !0}
			}, "hit")
		},
		...
		_hitExt: function (a, b) {
			...
			if (!f.isMetrikaPlayer()) {
				var k = a.modes || {};
				var l = a.browserInfo || {};
				var p = "undefined" !== typeof a.referrer ? a.referrer : oa.lastReferrer;
				var q = a.url || "";
				var n = {};
				a.nohit && c("nohit",
					1);
				k.ar && !k.onlyData && (p = null === p || "" === p ? "" : this._prepareHitUrl(p), q = this._prepareHitUrl(a.url));
				c("forceNoRedirect", a.forceNoRedirect);
				c("page-ref", f.trim(p, wa));
				c("page-url", f.trim(q, wa));
				f.is4Bytes(a.title) && c("charset", "utf-8");
				Ma() ? c("ut", Wa) : "undefined" != typeof k.ut && c("ut", f.trim(k.ut, Gb));
				c("exp", a.experiments);
				(e = a.vParams) && e.__ym && (f.isNumber(e.__ym.turbo_page) && (h.tp = e.__ym.turbo_page), f.isNumber(e.__ym.turbo_page_id) && (h.tpid = e.__ym.turbo_page_id));
				(!e || f.isObject(e)) && this.paramsQueue.length() &&
				f.inArray(["go", "hit", "pa"], b) && (e = this.paramsQueue.mixin(e));
				e && (q = f.toJSON(e), this.trimParams && q.length > Hb ? g = !0 : c("site-info", q));
		...
		_getTechInfo: function (a, b, c, d, e, h, k, r, R, p) {
			...
			n.isIframe() && m("ifr", "1");
			g.ia_document && g.ia_document.shareURL && g.ia_document.referrer && m("iia", "1");
			m("adb", ib.getVal());
			...
			for (d = 0; d < e.length; d++) u(e[d]);
			e = ["va", "vt", "sn", "sa", "he"];
			b.nb && e.push("cl");
			for (d = 0; d < e.length; d++) h = e[d], m(h, b[h]);
			e = new T({counterId: c});
			e.isEnabled() && (c = e.getStorageId(), d = e.get("reqNum"), p || (d ? d++ : d = 1), e.set("reqNum", d), e.get("reqNum") == d ? (m("ls", c), m("rqn", d)) : (e.remove("reqNum"), e.clearStorageId(), 1 < d && (m("ls", c), m("rqn", 0))));
			m("rn", f.random());
			m("hid", k);
			m("ds", Bb(R));
		...
		_getTitle: function (a) {
			a = "undefined" == typeof a ? (a = l.getDocumentTitle()) ? f.trim(a, Na) : "" : f.trim(a, Na);
			return a
		}
	});
	...
	var ra = B.inherit({
		sampling: 1, counterId: 26302566, _initComponent: function () {
			...
			Math.random() < this.sampling && this._sender.sendHit({url: G.href, title: "", referrer: "", vParams: a})
		}
	}), Ib = P.inherit({
		resource: "webvisor", retry: !0, postParams: ["wv-data"], sendPacket: function (a, b, c, d, e, h, k, l) {
			if (!a || !a.length) return !1;
			var m = Q.encode(a, !0);
			if (0 == m.indexOf("AAAAAAAAADw") && g.Error) {
				var p = Error();
				"string" == typeof p.stack && (new ra({sampling: .1})).log("bad visor packet 5", 1)
			}
			a = {
				rn: f.random(), "page-url": b,
				wmode: 0, "wv-type": 0, "wv-hit": c, "wv-part": d + 1, "wv-check": f.fletcher(a), "wv-data": m
			};
		...
		sendContent: function (a, b, c, d, e, f, g) {
			...
			a = {"wv-type": 1, "page-url": D().href, "wv-hit": c, "wv-data": Q.encode(na.encode(a))};
	...
	l.getDocumentScroll = function () {
		var a = l.getBody();
		return [g.pageXOffset || h.documentElement && h.documentElement.scrollLeft || a && a.scrollLeft || 0, g.pageYOffset || h.documentElement && h.documentElement.scrollTop ||
		a && a.scrollTop || 0]
	};
	l.getElementParent = function (a) {
		if (!a || a == h.documentElement) return null;
		if (a == l.getBody()) return h.documentElement;
		var b = null;
		try {
			b = a.parentNode
		} catch (c) {
	...
	l.getElementXY = function (a) {
		var b;
		if (!a || !a.ownerDocument || "PARAM" == a.tagName || a == l.getBody() || a == h.documentElement) return [0, 0];
	...
	l.getElementSize = function (a) {
		var b;
		return a == l.getBody() || a == h.documentElement ? l.getDocumentSize() : (b = a.getBoundingClientRect && a.getBoundingClientRect()) ? [b.width, b.height] : [a.offsetWidth, a.offsetHeight]
	};
	...
	l.calcTextChecksum = function (a) {
		...
		var d = 0;
		for (c =
			     a.length; d < c; d++) a[d] && 3 == a[d].nodeType && (b += a[d].nodeValue);
		return f.fletcher(b.replace(/[\u0000-\u0020]+/g, ""))
	};
	...
	l.getElementsByClassName = function (a, b) {
		...
		var e = [];
		for (c = 0; c < d.length; c++) l.classNameExists(d[c], a) && e.push(d[c]);
		return e
	};
	...
	var wb = !ia(/webkit/) && ia(/gecko/), jb = B.inherit({
		hid: 0, tz: 0, ts: 0, counterId: 0, counterType: 0,
		url: "", protocol: "", enabled: !1, _initComponent: function () {
			if (this.isEnabled()) {
				var a = this._getStorage();
				var b = a.read("visorc");
				"b" !== b && "w" !== b && (b = "");
				if (!a.isEnabled() || ia("opera mini")) b = "b";
				this._vc = b;
				"b" !== b && (this._recorder = new pb(this.protocol, this.counterId, this.counterType, this.hid, this.globalConfig));
	...
	ja = S.inherit({
		id: "script", index: 6, request: function (a,
		                                           b, c, d) {
			var e = !1, k = f.getNativeFunction("createElement", h)("script");
			this.timeout && f.setTimeout(function () {
				e || (e = !0, l.removeNode(k), c.call(d, !1))
			}, this.timeout, "ts.request");
			if (!k) return e = !0, c.call(d, !1), !1;
			...
			k.src = this._buildUrl(a, f.mixin({wmode: 5, callback: u}, b));
			k.async = "async";
	...
	var Oa = da.inherit({
		id: "XHRhit", index: 10, _buildUrl: function (a, b) {
			b.wmode = 7;
			return Oa.superclass._buildUrl.apply(this, [a, b])
		}, _parseResp: function (a) {
			var b = Y.parse(a);
			if (!b) throw"";
			return f.map(function (a) {
		...
		transports: [Oa, ja], trimParams: !0, _initComponent: function () {
			Xa.superclass._initComponent.apply(this,
				arguments);
			this._ilStorage = new T({counterId: this.counterId})
		}, postParams: [], checkYandexHost: function (a) {
			if (a) {
				a:{
					if (a = f.parseUrl(a).host.match(/(?:^|\.)(?:ya|yandex)\.(?:\w+|com?\.\w+)$/)) if (a = a[0].split("yandex").reverse()[0].substring(1), -1 != f.arrayIndexOf(I.yandexWhiteListTld, a)) break a;
					a = !1
				}
			...
			c && (this._ilStorage.remove("il"), b.ilt = Q.encode(na.encode(f.trim(c,
				kb))));
		...
		cid: 3, cnt: 0, KEY_NAME: "wasSynced", _initComponent: function () {
			...
			var b = this, c = this._storage.get(this.KEY_NAME), d = [], e = this.globalConfig.get("fakeHit");
			if (e) return e(function (b) {
				a(b)
			});
			c || (c = this._cookie.read(this.KEY_NAME)) && (c = Y.parse(c));
			c && c.time + 864E5 > w.getMs() ? a(c) : (this.globalConfig.set("fakeHit", function (a) {
				d.push(a)
			}), this._getSender().sendHit({
				callback: function (e, g) {
					c = {time: w.getMs(), params: e, bkParams: g};
					b.globalConfig.set("isEU", e.eu);
					b._storage.set(b.KEY_NAME, c);
					b._cookie.create(b.KEY_NAME, Y.stringify(c), 1728);
					a(c);
			...
			this._sender || (this._sender = new Xa({
				protocol: "https:", counterType: this.cnt, counterId: this.cid,
				sendTitle: !1, globalConfig: this.globalConfig, fake: !0
			}));
			return this._sender
		}
	}), lb = B.inherit({
		baseUrl: I.baseUrl, globalConfig: t, baseTld: "ru", _initComponent: function () {
			var a = this;
			lb.superclass._initComponent.apply(this, arguments);
			this.syncTlds = f.reduce(function (a, c) {
				return c(a)
			}, I.yandexWhiteListTld, [f.bind(f.filter, null, function (b) {
				return b !== a.baseTld
			...
			ka: "ge", ro: "md", tg: "tj", tk: "tm", et: "ee", hy: "com.am", he: "co.li", ky: "kg", uk: "ua",
			be: "by", tr: "com.tr", kk: "kz"
		}, sync: function (a) {
			var b = this, c = this._need(), d = Ma();
			!this._is() && c ? (this._setStatus(!0), d ? this._getCn().hit(function () {
				b._sync(c, a)
			}) : a()) : a();
			d && this._addMda()
		}, _addMda: function () {
			var a = new J({skipPrefix: !0}), b = new J, c = a.read("mda");
			...
			var a =
				this._getDomainByLang(), b = this._getTld(), c = [], d;
			...
			var c, d = 0, e = I.protocol + "//", f = a.length, g = this;
			if (0 < f) {
				b = b || function () {
				};
				var h = function () {
				...
				for (c = 0; c < f; c++) (function (a) {
					l.request(e + (g.baseUrl + a) + "/sync_cookie_image_check",
						{}, function (b) {
							h();
							var c = w.getMinutes();
							b || (c -= 1435);
			...
			this._cn || (this._cn = new Jb({fake: !0, globalConfig: this.globalConfig}));
			return this._cn
		}, _getLs: function () {
			...
			try {
				var a = navigator.languages ? navigator.languages[0] : n.getLanguage()
			} catch (b) {
			}
	...
	n.isAndroid = v(function () {
			var a = navigator.userAgent.toLowerCase(), b = n.getPlatform() || "";
			return -1 < a.indexOf("android") && -1 < a.indexOf("mobile") && b.match(/^android|linux armv/i)
		},
		"", !0);
	var pa = {
			_url: I.protocol + "//" + I.host + "/user_storage_set?", _params: {}, init: function (a) {
				this._params = a || {}
			}, save: function (a, b) {
				this._getReq().request(this._url + ca.stringify({key: a, value: f.toJSON(b)}));
				this._getLs().set(a, b)
			}, get: function (a) {
		...
		Lb = {
			sync: v(function (a, b, c, d) {
				var e = {t: 'UV|L7,!"T[rwe\x26D_\x3eZIb\\aW#98Y.PC6k'};
				this.globalConfig = d || t;
				if (this._enabled(a, b, c)) {
					this._setStatus(!0);
					var f = new Kb({method: "GET", withCredentials: !1});
					var g = this._getHosts();
					var h = g.length;
					(function q(a) {
						var b = g[a];
						a < h ? f.request(b[0] + ":" + b[1] + "/p", e, function (c, d) {
							c ? this._save(c, d, b[1]) : q.call(this, a + 1)
						}, this) : this._save(!1)
					}).call(this, 0)
				}
			}, "ds"), _getHosts: function () {
				var a = [], b = G.protocol, c = n.isIOS();
				(n.isAndroid() || c && "http:" === b) && a.push(["http://127.0.0.1",
					"30102"], ["http://127.0.0.1", "29009"]);
				...
				if (!(60 < w.getMinutes() - pa.getNum("ds")) || this._is() || f.isDev(this.globalConfig)) return !1;
				a = 0 === b && 0 <= f.arrayIndexOf([39370095, 1282253, 29302335, 43653484, 41734944, 31048081, 16443139, 27737346, 41870499, 33911514, 30744873, 5564395, 2580487], a);
				a = 0 == D().hostname.search(/^(.*\.)?((ya|yandex(-team)?)\.(com\.)?\w+|(auto|kinopoisk)\.ru|yadi\.sk|yastatic\.net)$/) || !c && .1 >= Math.random() && a;
				c = n.isIOS() && a;
			...
			getPos: function (a) {
				...
				return a.which || a.button == E ? a.which : a.button & 1 ? 1 : a.button & 2 ? 3 : a.button & 4 ? 2 : 0
			}, prevent: function (a) {
				a = a || g.event;
	...
	F.dispatchCustomEvent = v(function (a, b) {
		var c;
		b = b || h;
		if (c = f.getNativeFunction("createEvent",
			h)("Event")) c.initEvent(a, !1, !1), f.getNativeFunction("dispatchEvent", b)(c)
	}, "evt.dispatch");
	var oa = {lastReferrer: null};
	n.isIE = function () {
	...
	var la = {};
	la._cookie = new J({onlyCurrentDomain: !0});
	la.log = v(function () {
		var a = -1 < G.href.indexOf("_ym_debug\x3d1") || g._ym_debug;
		a && this._cookie.create("debug", "1");
		g.console && console.log &&
		(a || "1" === this._cookie.read("debug")) && console.log.apply(console, arguments)
	}, "DebugConsole.log");
	var Mb = P.inherit({
		resource: "clmap", retry: !0, transports: [V], sendClick: function (a, b, c, d) {
	...
	var Pa = B.inherit({
		...
		checkAssessor: function () {
			...
			f.forEachKey(b, function (c) {
				if ((new RegExp(c)).test(g.location.hash) || J.get(this.ASSESSOR_COOKIE + c)) J.set(this.ASSESSOR_COOKIE + c), a.push(b[c])
			}, this);
			return a
		}, _initComponent: function () {
			var a = this.checkAssessor();
			this.globalConfig.get(this.REMOTE_CONTROL) || (this.globalConfig.set(this.REMOTE_CONTROL, !0), Pa.superclass._initComponent.apply(this, arguments), this._executedMsgs = {}, k.on(g, "message", this._onMessage, this), a.length && (this.globalConfig.set("oo",
				!0), this.globalConfig.get("stopRecoder", function () {
			})(), f.arrayEvery(a, function (a) {
				l.loadScript({src: a}, g);
				return 1
			})))
		}, _buildRemoteIframe: function (a) {
			var b = f.getNativeFunction("createElement", h)("div"), c = l.getBody() || h.documentElement;
			if (c) {
				...
				var d = b.firstChild;
				d.onload = function () {
					l.loadScript({src: a}, d.contentWindow)
				};
				g._ym__remoteIframeEl =
					d;
		...
		_initComponent: function () {
			...
			(f.isString(this.prefsEcommerce) && (this.dataLayerName = this.prefsEcommerce), a = this.dataLayerName, this._tryTimeout = f.bind(this._tryTimeout, this), this._tryTimeout()), this.counter._ecommerce = a
		},
		...
		_send: function (a) {
			...
			for (e = a.length; h < e; h++) {
				if ((d = a[h]) && d.ymetrikaEvent && d.ymetrikaEvent.data) {
					...
					"params" === k.type && (k.sended || (k.sended = []), k.counter ? this.counter && this.counter.params && this.counterId == k.counter && (this.counter.params(k.data), k.sended.push(this.counterId)) : f.forEachKey(this.globalConfig.get("counters"), function (a, b) {
						var c = a.split(":")[0];
						!f.inArray(k.sended, c) && b && b.params && (b.params(k.data), k.sended.push(c))
					}, this), k.status ==
					this.status.NEW && k.parent && this.frameConnector && this.frameConnector.checkFrame(g.parent) && g.parent !== g && (this.frameConnector.sendToParents({
						type: "params",
						data: k.data
					}, function () {
		...
		(function (a) {
			...
			b.d = function (a, c, f) {
				b.o(a, c) || Object.defineProperty(a,
					c, {configurable: !1, enumerable: !0, get: f})
			};
			b.n = function (a) {
			...
			3: function (a, b, c) {
				...
				b.__esModule = !0;
				var d = c(5), e = c(9), f = c(34), k = c(6), l = h.body, n = h.documentElement;
				b.detectAd = function () {
					var a = g.pageYOffset || n.scrollTop || l.scrollTop,
						b = g.pageXOffset || n.scrollLeft || l.scrollLeft, c = n.clientTop ||
						l.clientTop || 0, h = n.clientLeft || l.clientLeft || 0,
						m = n.scrollWidth || n.offsetWidth || g.innerWidth,
						r = n.scrollHeight || n.offsetHeight || g.innerHeight, u = k.getFrienlyWindowsTree(g), t = [];
					k.windowsTreeEach(u, function (a) {
						var b = a.window, c = a.parent,
							f = 0 < d.getParentFriendlyIFrames(b.document.documentElement).length,
							g = e.getAdsInWindow(b, {isInIFrame: f});
						f = g.adElements;
						g = g.iframeProviders;
						if (c) {
							var h = b.frameElement, k = t.filter(function (a) {
					...
					t.forEach(function (a) {
						a.adElements = a.adElements.sort(function (a, b) {
							return f.getParentElementsCount(a.element) - f.getParentElementsCount(b.element)
						})
					});
					var v = [], ma = [];
					t.forEach(function (a) {
						...
						a.forEach(function (a) {
							var b = a.element;
							ma.push({provider: a.provider, element: c ? e : b})
						})
					});
					ma.forEach(function (d) {
						...
						var k = e.clientWidth;
						e = e.clientHeight;
						var l = f + k, p = g + e, n = 0 > f ? 0 : f < m ? f : m, q = 0 > g ? 0 : g < r ? g : r;
						100 <= ((l > m ? m : l > n ? l : n) - n) * ((p > r ? r : p > q ? p : q) - q) && v.push({
							top: g,
							left: f,
							width: k,
							height: e,
			...
			6: function (a, b, c) {
				...
				b.getAdsInWindow = function (a, b) {
					...
					var f = b.isInIFrame, g = a.document.body, h = d.listToArray(g.querySelectorAll("img")),
						k = d.listToArray(g.querySelectorAll("iframe")),
						l = d.listToArray(g.querySelectorAll("script")), m = d.listToArray(g.querySelectorAll("div")),
						n = [], u = [], t = d.listToArray(g.querySelectorAll("yatag")),
					...
					c("criteo", k.filter(function (a) {
						...
						a = a.id;
						return b &&
						(-1 < b.indexOf(".criteo.com/") || a && -1 < a.indexOf("cto_iframe_")) ? !0 : !1
					}));
	...
	var Pb = {
			...
			checkParentData: function (a, b) {
				f.arrayEvery(this.dataToSend.parents, function (c) {
					-1 == f.arrayIndexOf(c.tryTo, b.counterId) && (c.tryTo.push(b.counterId), f.mixin(c.data, {toCounter: b.counterId}), this.sendToIframe(a,
						c.data, function () {
			...
			genMessage: function (a, b) {
				var c = {date: w.getMs(), key: Math.random(), dir: 0};
				b && (c = {
					date: b[0],
					key: b[1], dir: b[2]
				});
				f.mixin(a, {counterId: this.counterId, hid: this.hid});
				var d = {data: a};
			...
			sendToIframe: function (a, b, c, d) {
				b = this.genMessage(b);
				var e = this, g = [b.meta.date, b.meta.key].join(this.spliter);
				this.checkFrame(a) && (a.postMessage(b.string, "*"), this.pending[g] = f.bind(c, d || this), f.setTimeout(function () {
					delete e.pending[g]
				}, 5E3, this, "fc.toSendToIframe"))
			},
			sendToParents: function (a, b, c) {
				var d =
					this.dataToSend.parents.push({sendedTo: [], tryTo: [], callback: f.bind(b, c || this), data: a}) - 1;
				f.forEachKey(this.parents, function (a) {
					this.dataToSend.parents[d].tryTo.push(a);
					return 1
				}, this);
				f.keys(this.parents).length && this.sendToIframe(g.parent, a, function (a) {
			...
			_initComponent: function () {
				...
				if (this.message) {
					for (b = 0; b < a.length; b++) {
						var c = a[b];
						(function (a, b) {
							a.sendToIframe(b.contentWindow, {type: "initToChild"}, function (c) {
								a.childs[c.counterId] = {info: c, window: b.contentWindow}
							})
						})(this, c)
					}
					n.isIframe() && this.sendToIframe(g.parent, {type: "initToParent"}, function (a) {
						this.trigger("parentConnect", {source: g.parent},
							a)
					});
					k.on(g, "message", this.onMessage, this, {passive: !1, name: "FrameConnector"})
				}
			},
			trigger: function (a, b, c) {
				...
				return f.map(function (a) {
					...
					return f.mixin(a, {counterId: d.counterId, hid: d.hid})
				}, this.handlers[a])
			},
			DIR_INDEX: 2,
			onMessage: function (a) {
				...
				if (e && e.substring && e.substring(0, this.nameSpace.length) == this.nameSpace) {
					...
					} else b[this.DIR_INDEX] == this.counterId && f.isArray(d) && f.arrayEvery(d, function (a) {
						return a.counterId && a.hid
					}) && (a = this.pending[[b[0], b[1]].join(this.spliter)]) &&
					a.apply(this, d)
				}
			}
		}), G = D(), Aa = w.getTimezone(), Ba = w.getTimestamp(), U = g.screen, Gb = 64, wa = n.isIE() ? 512 : 2048,
		Hb = n.isIE() ? 512 : 2048, Na = n.isIE() ? 100 : 400, kb = 100, Wa = "noindex",
		...
		Qb = +new Date, Za, qa, Z = function () {
			return "1112"
		}();
	t.init().setSafe("counters", {}).setSafe("hitParam", {}).setSafe("counterNum", 0).setSafe("dataLayer", []).setSafe("hitId", f.random(1, 1073741824)).setSafe("_globalMetrikaHitId", f.random()).setSafe("v", Z);
	g.Ya.Metrika = function (a, b, c, d) {
		var e = this;
		return v(function () {
			function m(a) {
				var b = h.referrer || "", c, d;
				if ((new RegExp("^https?://" + G.host + "/")).test(b)) return !1;
				var e = a.patterns;
				for (c = 0; c < e.length; c++) {
					var g = new RegExp(e[c], "i");
					if (b.match(g)) if (g = a.params ||
						[], g.length) {
						var k = f.safeDecodeURIComponent((RegExp.$1 || "").replace(/\+/g, "%20"));
						for (d = 0; d < g.length; d++) if (k == f.safeDecodeURIComponent(g[d])) return !0
					} else return !0
				}
				return !1
			}
			...
			function u(a, b) {
				...
				if (a && "string" != typeof a && a.length) for (d = 0; d < a.length; d++) {
					...
					e = e.slice(1);
					if ("#" == g) {
						if (g = h.getElementById(e)) c = !0, b && fa.unshift([g, g.innerHTML]), g.innerHTML = f
					} else if ("." == g) for (g = l.getElementsByClassName(e), e = 0; e < g.length; e++) {
						c = !0;
						var k = g[e], y = f;
			...
			function I(a, b) {
				...
				for (c = 0; c < h.length; c++) {
					var k = h[c].split("\x26");
					for (e = 0; e < k.length; e++) g = k[e].split("\x3d"), "_openstat" === g[0] && (d = g[1])
				}
				d && (d = -1 < d.indexOf(";") ? f.safeDecodeURIComponent(d) :
			...
			function p(a, b) {
				return v(function () {
					if (!e.isDestructing()) return a.apply(this, arguments)
				}, b || "")
			}
			...
			function q(y) {
				...
				if (t.get("hitParam")[aa]) {
					if (1 != c || t.get("counters")[aa]) return !1;
					A = !0
				}
				t.get("counters")[aa] = e;
				t.get("hitParam")[aa] = 1;
					...
					var L = function (A) {
						...
						L.sendHit({
							...
							callback: function (d, y) {
								...
								Ob.match();
								Lb.sync(a, c, l, t);
								la.log("PageView. Counter ", a, ". URL: ", G.href, ". Referrer: " + h.referrer, ". Params: ", b);
								qa || (qa = +new Date);
									...
									if (n) {
										b:{
											var L =
												n.conditions;
											var q;
											if (L && L.length) for (q = 0; q < L.length; q++) {
												var p = L[q];
												if ("ref" == p.type) p = m(p); else if ("adv" == p.type) {
													var H;
													var O = p;
													var v = Ya._metrika.counter._directCampaign;
													var X = O.ServiceNamePattern;
													var w = O.RefererPattern;
													p = v ? O.direct_orders : O.direct_camp;
													var B = h.referrer, z = ca.parse(G.search), C = I(G.search, G.hash),
														D = {};
													var x = ["source", "medium", "campaign", "term", "content"];
													for (H = 0; H < x.length; H++) z["utm_" + x[H]] && (D[x[H]] = z["utm_" + x[H]]);
													var F = v ? "direct.yandex.ru" : C && C.service || D.source;
													H = !1;
													if (!H && X &&
														X.length) for (x = 0; x < X.length; x++) if ((new RegExp(X[x], "i")).test(F)) {
														H = !0;
														break
													}
													if (!H && !O.yandex_direct && w && w.length) for (X = 0; X < w.length; X++) if ((new RegExp(w[X], "i")).test(B)) {
														H = !0;
														break
													}
													!H && O.google_adwords && z.gclid && (H = !0);
													if (H && p && p.length && (H = !1, v = v || C && C.campaign || D && D.campaign)) for (O = 0; O < p.length; O++) if (p[O] == v) {
														H = !0;
														break
													}
													p = H
										...
										L && n ? (l.create("mp2_substs", f.toJSON(n)),
											l = u(n), e.params("__ym", l ? "mp_trackid" : "mp_trackid_bad", L)) : r()
									} else r();
				...
				B(!1);
				k.on(h, "click", V);
				z && (z.enableAll ? (B(!0), S(!0), U()) : (z.clickmap && S(z.clickmap), z.trackLinks && B(z.trackLinks), z.accurateTrackBounce && U(z.accurateTrackBounce)), new mb({
					counter: e,
					counterId: a,
					globalConfig: t,
					frameConnector: Sa,
			...
			function Z(a) {
				...
				if (b) {
					...
					var e = function (a) {
						var d = fb(b);
						N.sendClickLink({url: c, title: c === d ? "" : d, modes: a})
					};
					if (ob.test(d) || ob.test(c) || gb(c, ha) || gb(d, ha)) a = !0;
					var g = l.classNameExists(b, "ym-disable-tracklink");
					d = l.classNameExists(b, "ym-external-link");
					g || (g = {
						ln: !0,
						dl: a
					}, d ? e(g) : (d = b.hostname || f.parseUrl(b.href).hostname || "", zb(D().hostname, d) ? a ? (g.ln = !1, e(g)) : (a = fb(b)) && a != c && Na.set("il", f.trim(a, kb)) : c && -1 != c.search(/^ *(data|javascript):/i) || (g.ut = Wa, e(g))))
				}
			}
			...
			function da(a,
				...
				function e(a) {
					var b;
					for (b = 0; b < t.length; b += 3) if (a) k.on(t[b], t[b + 1], t[b + 2], {name: "tabTrack"}); else k.un(t[b], t[b + 1], t[b + 2])
				}

				var l = !1, A = !1, y = !0, m = 0, p = +new Date, q = null, r = !1;
				if (n.isIE()) f.setTimeout(a, b, "trackUserTime"); else {
			...
			function U(b) {
				...
				if (!e._isAccurateTrackBounce) {
					...
					((c = c.get("lastHit")) && (!d || d < c - 18E5) || !Ab(h.referrer, D().href) || .1 > Math.random()) && da(function () {
						e.notBounce()
					}, b)
				}
			}
			...
			function ma(a) {
				if (!1 === a) ba && ("onhashchange" in g ? k.un(g, "hashchange", ea) : f.clearTimeout(ta), ba = !1); else if (a = ea, !ba) {
					if ("onhashchange" in g) k.on(g, "hashchange", a); else ia(a);
					ba = !0
				}
				N.setTrackHash(ba);
				e._trackHash = ba
			...
			function ea() {
				var a = {ut: ja, ad: 1 == c && g.Ya && g.Ya.Direct, wh: !0, saveRef: !0, pv: !0};
				wa = oa.lastReferrer = xa;
				N.sendAjaxHit({
					url: D().href, title: l.getDocumentTitle(), referrer: wa,
					modes: a
				});
				xa = D().href
			}
			...
			function ua(a, b, c) {
				...
				b = [];
				a && (b.push(a), c = c || {}, f.isFunction(c.callback) && (b.push(c.callback), c.ctx && b.push(c.ctx)));
				b.length && e.params.apply(e, b)
			}

			var ja = "", wa = "", xa = oa.lastReferrer = G.href, sa;
			t.setSafe("counter", e);
			if ("object" === typeof a) {
				var z = a;
				d = a.defer;
			...
			if (!g["disableYaCounter" + a]) {
				...
				(function (a) {
					if (g.performance && "function" === typeof performance.getEntries && (a === E && (a = 1), !(Math.random() >= a))) {
						...
						f.keys(c).length && (new ra).logParams({timings8: c})
					}
				})(24226447 ===
				a ? 1 : .002);
				C = C ? C : G.href;
				...
				c = c || 0;
				var aa = a + ":" + c;
				if (t.get("counters")[aa]) return t.get("counters")[aa];
				var W = t.get("hitId");
				...
				e.replacePhones = p(function () {
					var b, c;
					try {
						(b = (new J({counterId: a})).read("mp2_substs")) &&
						(c = Y.parse(b)) && u(c, !0)
					} catch (L) {
					}
					return e
				...
				e.reachGoal = p(function (b, c, d, f) {
					...
					N.sendGoal(b, {vParams: c, callback: d, ctx: f});
					return e
				}, "counter.reachGoal");
				...
				e.hit = p(function (b, c, d, g, h, k) {
					b ? (f.isObject(c) && (d = c.referer, g = c.params, h = c.callback, k = c.ctx, c = c.title), la.log("PageView. Counter " +
						a, ". URL: " + b, ". Referrer: " + d, ". Params: ", g), N.sendHit({
						url: b,
						title: c,
						referrer: d,
						vParams: g,
				...
				e.params = p(function (b) {
					var c = arguments.length, d = c, g = [].slice.call(arguments, 0);
					if (b) {
						...
						c = -1 !== f.arrayIndexOf(f.keys(g[0]),
							"__ymu");
						la.log((c ? "User p" : "P") + "arams. Counter " + a + ". Params: ", g[0]);
				...
				Sa.addHandler("parentConnect", function (a, b) {
					N.paramsQueue.push({__ym: {parentIframe: {counterId: b.counterId, hid: b.hid}}})
				}, this);
				...
				e.notBounce = p(function (a) {
					...
					a = a || {};
					Za && qa && (b = qa - Za);
					N.sendNotBounce(b, {callback: a.callback, ctx: a.ctx});
					return e
				}, "counter.notBounce");
				var ha = [];
				e.addFileExtension = p(function (a) {
					if (!a) return f.warning("Empty extension"), e;
					"string" === typeof a ? ha.push(a) : ha = ha.concat(a);
					return e
				});
				...
				e.destruct = p(function () {
					...
					S(!1);
					try {
						delete g["yaCounter" + a], delete t.get("counters")[aa]
					} catch (y) {
					}
				});
				...
				if (a) if ("prerender" === h.webkitVisibilityState) {
					N.sendPrerenderHit({url: G.href, title: l.getDocumentTitle(), referrer: h.referrer});
					var bb = function () {
						"prerender" !== h.webkitVisibilityState && (k.un(h, "webkitvisibilitychange", bb), q(!0))
					};
					k.on(h, "webkitvisibilitychange",
						bb)
				} else q(!1)
			}
		}, "init")()
	...
	(function () {
		...
		var b = g.yandex_metrika_callback,
			c = g.yandex_metrika_callbacks, d;
		"function" == typeof b && b();
